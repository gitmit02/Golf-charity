          ...
          <div className="md:col-span-5 space-y-6">
            ...
            <div className="flex gap-4">
              {socialLinks.map((social, i) => (
                <a key={i} href={social.href} className="w-10 h-10 rounded-xl bg-slate-800/50 border border-slate-700 flex items-center justify-center text-slate-400 hover:text-emerald-400 hover:border-emerald-500/50 transition-all">
                  <social.icon size={18} />
                </a>
              ))}
            </div>
            ...
            <ul className="space-y-4">
              {links.map((link) => (
                <li key={link.path}>
                  ...
                  >
                    {/* Replaced bar with Chevron icon */}
                    <ChevronRight className="w-0 group-hover:w-4 h-4 text-emerald-500 opacity-0 group-hover:opacity-100 transition-all duration-300" />
                    {link.name}
                  </Link>
                </li>
              ))}
          ...
          <div className="md:col-span-4 bg-slate-800/30 p-8 rounded-[2rem] border border-slate-800/50">
            ...
            <ul className="space-y-5">
              <li className="flex items-center gap-4 group cursor-pointer">
                <div className="w-10 h-10 rounded-xl bg-slate-800 flex items-center justify-center group-hover:bg-emerald-500 transition-all">
